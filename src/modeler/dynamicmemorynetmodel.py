...
class DynamicMemoryNet(TFModel):
    def __init__(self, num_classes, learning_rate, batch_size, decay_steps, decay_rate, sequence_length, story_length,
                 vocab_size, embed_size, hidden_size, is_training, num_pass=2, use_gated_gru=True,
                 decode_with_sequences=False, multi_label_flag=False,
        ...
        self.multi_label_flag = multi_label_flag
        super(DynamicMemoryNet, self).__init__()
    ...
    def build(self):
        ...
        if not self.multi_label_flag:
            ...
            # fuke accuracy. (you can calcuate accuracy outside of graph using method calculate_accuracy(...) in train.py)
    ...
    def input_module(self):
        ...
        self.story_embedding, hidden_state = tf.nn.dynamic_rnn(cell, story_embedding, dtype=tf.float32,
                                                               scope="input_module")
    ...
    def question_module(self):
        ...
        cell = rnn.GRUCell(self.hidden_size)
        _, self.query_embedding = tf.nn.dynamic_rnn(cell, query_embedding, dtype=tf.float32,
                                                    scope="question_module")  # query_embedding:[batch_size,hidden_size]
    ...
    def episodic_memory_module(self):  # input(story):[batch_size,story_length,hidden_size]
        ...
        # a list. length is: story_length. each element is:[batch_size,1,embedding_size]
        candidate_list = [tf.squeeze(x, axis=1) for x in candidate_inputs]
        # a list. length is: story_length. each element is:[batch_size  ,embedding_size]
        ...
        for pass_number in range(self.num_pass):  # for each candidate sentence in the list,do loop.
            ...
            g = self.attention_mechanism_parallel(self.story_embedding, m_current, self.query_embedding, pass_number)
            # [batch_size,story_length]
    ...
    def answer_module(self):
        ...
        # decoder for a list of tokens with sequence. e.g."x1 x2 x3 x4..."
        a = self.m_T  # init hidden state
        y_pred = tf.zeros((self.batch_size, self.hidden_size))
        # TODO usually we will init this as a special token '<GO>',
        # you can change this line by pass embedding of '<GO>' from outside.
        logits_list = []
        logits_return = None
    ...
    def attention_mechanism_parallel(self, c_full, m, q, i):
        ...
        z = tf.concat([c_full, m_tile, q_tile, c_q_elementwise, c_m_elementwise, c_q_minus, c_m_minus, c_w_q, c_w_m],
    ...
    def x1Wx2_parallel(self, x1, x2, scope):
        ...
        with tf.variable_scope(scope):
            x1 = tf.reshape(x1, shape=(self.batch_size, -1))
            # [batch_size,story_length*hidden_size]
            x1_w = tf.layers.dense(x1, self.story_length * self.hidden_size, use_bias=False)
            # [self.hidden_size, story_length*self.hidden_size]
